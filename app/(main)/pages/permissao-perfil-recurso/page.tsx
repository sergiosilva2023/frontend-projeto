...
const PermissaoPerfilRecurso = () => {
            ...
            permissaoPerfilRecursoService.inserir(permissaoPerfilRecurso)
                .then((response) => {
                    ...
                    toast.current?.show({
                        severity: 'info',
                        summary: 'Sucesso!',
                        detail: 'Permissão registada com sucesso!'
                    });
                }).catch((error) => {
        ...
        if (permissaoPerfilRecurso.id) {
            permissaoPerfilRecursoService.excluir(permissaoPerfilRecurso.id).then((response) => {
                ...
                toast.current?.show({
                    severity: 'success',
                    summary: 'Sucesso!',
                    detail: 'Permissão excluida com sucesso!',
                    life: 3000
                });
            }).catch((error) => {
                toast.current?.show({
    ...
    const deleteSelectedPermissaoPerfilRecursos = () => {
        ...
        Promise.all(selectedPermissaoPerfilRecursos.map(async (_permissaoPerfilRecurso) => {
            ...
            toast.current?.show({
                severity: 'success',
                summary: 'Sucesso!',
                detail: 'Permissões excluidas com sucesso!',
                life: 3000
            });
        }).catch((error) => {
            toast.current?.show({
    ...
    const rightToolbarTemplate = () => {
        return (
            <React.Fragment>
                ...
                <Button label="Exportor" icon="pi pi-upload" severity="help" onClick={exportCSV} />
            </React.Fragment>
        );
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Gestão de Permissão de Perfil</h5>
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={deletePermissaoPerfilRecursoDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deletePerfilDialogFooter} onHide={hideDeletePermissaoPerfilRecursoDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {permissaoPerfilRecurso && (
                                <span>
                                    Deseja realmente excluir a permissão <b>{permissaoPerfilRecurso.id}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deletePermissaoPerfilRecursosDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deletePerfilsDialogFooter} onHide={hideDeletePermissaoPerfilRecursosDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {permissaoPerfilRecurso && <span>Deseja realmente excluir as permissões selecionadas?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
